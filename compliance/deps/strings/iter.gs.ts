	...
	return (_yield: ((p0: string) => boolean) | null): void => {
		for (; $.len(s) > 0; ) {
			let line: string = ""
			{
				...
				if (i >= 0) {
					[line, s] = [$.sliceString(s, undefined, i + 1), $.sliceString(s, i + 1, undefined)]
				} else {
					[line, s] = [s, ""]
				}
			}
...
export function splitSeq(s: string, sep: string, sepSave: number): iter.Seq<string> {
	if ($.len(sep) == 0) {
		return explodeSeq(s)
	}
	return (_yield: ((p0: string) => boolean) | null): void => {
...
// but without constructing the slice.
export function FieldsFuncSeq(s: string, f: ((p0: number) => boolean) | null): iter.Seq<string> {
	return (_yield: ((p0: string) => boolean) | null): void => {
		let start = -1
		for (let i = 0; i < $.len(s); ) {
			let size = 1
