...
-   **GoScript & Divergences:**
    ...
    -   **`new(T)`:**
        ...
        -   **TypeScript:** No direct equivalent.
        -   **Divergence:** GoScript translates `new(T)` into code that creates an instance of the TypeScript type corresponding to `T`, ensuring it's properly zero-initialized, and then returns a "pointer" to it (often a boxed value or a direct reference if `T` is a class). See `gs.New` in runtime.
    -   **Pointers and Addressability:**
        ...
        -   **Divergence:** This is a fundamental difference. GoScript simulates pointers using:
            -   Boxing: Wrapping primitive types or structs in a container object (e.g., `gs.Box<T>`) to allow shared references and `nil` representation.
            -   Direct References: For types that are already reference types in TypeScript (classes, arrays, maps).
            -   The `&` operator is translated to a boxing operation or taking a reference. The `*` operator is translated to unboxing or dereferencing. This is handled in `compiler.WriteStarExpr` and `compiler.WriteUnaryExpr` (for `&`).
...
-   **GoScript & Divergences:**
    -   **General Mapping:** The core of GoScript is mapping Go types to TypeScript types.
        ...
        -   **Pointer Types:** `*T` -> `gs.Box<T>` for boxing, or the direct TypeScript reference type if `T` maps to a class. `compiler.WritePointerType`.
...
-   **GoScript & Divergences:**
    -   **Mapping:** Go `*T` is mapped by `compiler.WritePointerType`.
        -   If `T` maps to a TypeScript primitive or a Go struct (which becomes a TS class designed for value semantics), `*T` typically maps to `gs.Box<TranslatedT>` from `@goscript/builtin`. This wrapper simulates indirection, shared mutability, and `nil`.
    ...
    -   **`nil` Value:** `gs.Box<T>` can be `null` (or have an internal `null` value) to represent Go's `nil` pointer. Standard TypeScript reference types can also be `null`.
    -   **Dereferencing (`*p`):** Translated to `p.value` for `gs.Box` or direct access if `p` is a direct reference.
    -   **Address-of (`&v`):** Translated to `new gs.Box(v)` for values needing boxing, or taking the reference directly if `v` is already a reference type suitable for Go's pointer semantics in that context.
    -   **Divergence:** Fundamental. TypeScript lacks Go's explicit memory addresses and pointer arithmetic. GoScript simulates pointer semantics through boxing and reference management. `unsafe.Pointer` is a significant challenge and generally unsupported or highly restricted.
...
-   **GoScript & Divergences:**
    ...
    -   **Runtime Behavior (Type Assertions/Switches):**
        ...
        -   **Divergence:** This is a major area. GoScript implements runtime type information (`gs.typeInfo` attached to values or `gs.Box`) and helper functions (`gs.typeAssert`, `gs.typeSwitch`) to emulate Go's type assertions (`x.(T)`) and type switches. This is handled by `compiler.WriteTypeAssertExpr` and `compiler.WriteStmtTypeSwitch`.
...
-   **GoScript & Divergences:**
    -   **Mapping Defined Types (Nominal Typing):**
        ...
        -   `type MyInt int` -> `class MyInt extends gs.NumberBox<MyInt> { ... }` or similar, to allow methods and distinctness. For simple types like `int`, it might use a branded type if no methods are expected and boxing is undesirable. However, for consistency and method attachment, classes are more robust. The `DESIGN.md` mentions nominal typing for defined types.
    -   **Method Inheritance:**
...
-   **GoScript & Divergences:**
    ...
    -   **Closures:**
        ...
        -   **Divergence (Potentially Major for mutable captures):**
            -   If a Go closure captures a variable by reference and modifies it, and that variable is a primitive type or a struct value type, Go uses a heap-allocated "box" for the variable.
            ...
            -   GoScript's `analysis.NeedsBoxed` and related functions detect variables that need to be heap-allocated (boxed) due to closure semantics. The generated TS code then uses `gs.Box<T>` for these variables (e.g., `let x = new gs.Box(0); ... closure uses x.value ...`). This is a significant transformation to ensure Go's closure semantics for mutable captured variables are preserved.
    -   **Overall:** Function literals map well to TS arrow functions or function expressions. The primary complexity is emulating Go's closure capture semantics for mutable variables, which GoScript handles via boxing analysis and runtime `gs.Box` helpers.
...
-   **GoScript & Divergences:**
    ...
    -   **Receiver Evaluation and Storage:**
        ...
        -   **Pointer Receiver on Addressable Value Type (`t.Mp` where `t` is `T`):
            ...
            -   TS: This is tricky. If `t` is a class instance, `t` is already a reference. If `t` is a primitive that needs boxing for addressability, `const box_t = gs.box(t); const f = (...args) => box_t.value.Mp(...args);` (assuming `Mp` is on the boxed value's type, which is complex). More likely, `const f = t.Mp.bind(t)` if `Mp` is correctly on `T`'s prototype and handles value/pointer semantics internally.
    ...
    -   `compiler.WriteSelectorExpr` needs to be aware of these nuances to generate the correct binding or wrapper function, potentially involving `gs.clone` or `gs.boxValueReceiverMethod` helpers.
...
-   **GoScript & Divergences (Continued):**
    ...
    -   **`t.Mp` (Pointer Receiver, Addressable Value Instance):**
        ...
        -   If `t` were a basic type or struct needing boxing to be addressable for a pointer receiver method, Go handles this. GoScript would need to reflect this, possibly by boxing `t` if `Mp` expects a pointer to a primitive. However, methods are typically on defined (struct/named) types. For a struct `T` mapped to a class, `t.Mp.bind(t)` is idiomatic.
...
-   **GoScript & Divergences (`compiler.WriteSelectorExpr` for method values):**
    ...
    -   **Addressability for Shorthand:**
        -   Go: `t.M` can mean `(&t).M`.
        -   TS: No direct addressability concept. If `t` is a value type in GoScript (e.g. plain object for struct), and `M` expects a pointer receiver, GoScript would need to handle the implicit `&t` when `t.M` is evaluated to create the method value. This means the method value for `t.M` would capture the *boxed* (pointer) version of `t`.
...
-   **GoScript & Divergences (Continued):**
    ...
    -   **Method Call `x.m()` with Auto-Addressing:**
        ...
        -   If `Point` is a struct (value type) and GoScript maps it to an object literal or a class that needs explicit boxing for pointer receivers, `WriteCallExpr` (via `WriteSelectorExpr` logic) needs to handle this. `gs.callMethod(p, "Scale", [3.5])` could manage this, taking address if needed.
