...
export function ContainsFunc(s: string, f: ((r: number) => boolean) | null): boolean {
	if (!f) return false
	for (const char of s) {
...
export function IndexFunc(s: string, f: ((r: number) => boolean) | null): number {
	if (!f) return -1
	let index = 0
	for (const char of s) {
		const codePoint = char.codePointAt(0)
...
export function LastIndexFunc(s: string, f: ((r: number) => boolean) | null): number {
	if (!f) return -1
	const chars = [...s]
...
export function Fields(s: string): $.Slice<string> {
	return $.arrayToSlice(s.trim().split(/\s+/).filter(part => part.length > 0))
}

// FieldsFunc splits the string s at each run of Unicode code points c satisfying f(c) and returns an array of slices of s.
export function FieldsFunc(s: string, f: ((r: number) => boolean) | null): $.Slice<string> {
...
export function Map(mapping: ((r: number) => number) | null, s: string): string {
	if (!mapping) return s
...
export function TrimLeft(s: string, cutset: string): string {
	return TrimLeftFunc(s, (r: number) => cutset.includes(String.fromCodePoint(r)))
}

// TrimRight returns a slice of the string s with all trailing Unicode code points contained in cutset removed.
export function TrimRight(s: string, cutset: string): string {
	return TrimRightFunc(s, (r: number) => cutset.includes(String.fromCodePoint(r)))
}

// TrimFunc returns a slice of the string s with all leading and trailing Unicode code points c satisfying f(c) removed.
export function TrimFunc(s: string, f: ((r: number) => boolean) | null): string {
	if (!f) return s
	return TrimRightFunc(TrimLeftFunc(s, f), f)
}

// TrimLeftFunc returns a slice of the string s with all leading Unicode code points c satisfying f(c) removed.
export function TrimLeftFunc(s: string, f: ((r: number) => boolean) | null): string {
	if (!f) return s
...
export function TrimRightFunc(s: string, f: ((r: number) => boolean) | null): string {
	if (!f) return s
