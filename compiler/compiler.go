...
// It returns the struct type, whether it's a struct literal, and whether it's an anonymous struct.
// If it's a named struct (not anonymous), it also writes the constructor prefix.
func (c *GoToTSCompiler) identifyStructType(exp *ast.CompositeLit, litType types.Type) (*types.Struct, bool, bool) {
	...
	return structType, isStructLiteral, isAnonymousStruct
}
...
func (c *GoToTSCompiler) WriteCompositeLit(exp *ast.CompositeLit) error {
	...
	if exp.Type != nil {
		...
		if arrType, isArrayType := exp.Type.(*ast.ArrayType); isArrayType {
			...
			structType, isStructLiteral, isAnonymousStruct := c.identifyStructType(exp, litType)
